...
waiting time is `r round(mean(waiting_times))` minutes.
...
```{r}
wait_threshold <- 30
customer_excluded <- scen2_sim_results_by_minute$queue_size[nrow(scen2_sim_results_by_minute)]+sum(scen2_sim_results_by_customer$waiting_time > wait_threshold)
num_customer <- nrow(scen2_sim_results_by_customer) - customer_excluded
expected_profit <- 50 * num_customer - 40 * 12* num_chefs
expected_profit
=======
...
total_customers <- length(arrival_times) #55
...
>>>>>>> c622df4e5b627dda20a136080a84bae9a474e79b
```
